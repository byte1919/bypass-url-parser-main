...
```
Bypass Url Parser, made with love by @Security_byt3
...
```bash
docker build -t security_byt3/bypass-url-parser:latest .
docker run --rm -it --net=host -v "$PWD:/host" security_byt3/bypass-url-parser:latest -u http://127.0.0.1:8000/dummy
docker run --rm -it --net=host -v "$PWD:/host" security_byt3/bypass-url-parser:latest -u /host/urls.lst
# TODO later, tag & push releases to dockerhub :)
# -t security_byt3/bypass-url-parser:0.2.0
# docker push security_byt3/bypass-url-parser:latest
```
